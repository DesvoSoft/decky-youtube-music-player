...
const PluginContentWrapper = () => {
  const { authenticated, hasCredentials } = usePlayer();
  const [authLoaded, setAuthLoaded] = useState(false);
  const [activeTab, setActiveTab] = useState<string>('player');

  useEffect(() => {
    if (hasCredentials || !authenticated) {
      setAuthLoaded(true);
    }
  }, [authenticated, hasCredentials]);

  // Show loading until auth state is confirmed
  if (!authLoaded) {
    return (
      <div style={{ textAlign: 'center', padding: '32px', color: 'var(--gpSystemLighterGrey)', fontSize: '13px' }}>
